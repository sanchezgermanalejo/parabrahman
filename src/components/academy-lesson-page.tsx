...
type AcademyLessonPageProps = {
  lessonId: (typeof academyCourse.lessons)[number]["id"];
  introduction: string;
  unitNote: string;
};
            ...
            <div className="py-8">
              ...
              <aside className="mt-8 rounded-2xl border border-sky-300/15 bg-sky-300/5 p-5 text-sm leading-6 text-stone-400">
                <strong className="text-stone-200">
                  Unidad audiovisual {lesson.order}:
                </strong>{" "}
                {unitNote}
              </aside>
          ...
          <aside className="luminous-card h-fit rounded-3xl border border-amber-100/10 bg-stone-900/70 p-5 lg:sticky lg:top-28">
            ...
            <div className="mt-5 grid gap-3">
              {academyCourse.lessons.map((item) => {
                ...
                const content = (
                  <>
                    <p className="text-xs text-stone-500">Lección {item.order}</p>
                    <p className="mt-1 font-medium text-stone-200">{item.title}</p>
                    {!item.available && (
                ...
                return item.available && item.href.startsWith("/") ? (
                  <Link
                    key={item.id}
                    href={item.href}
                    className={`${className} hover:border-amber-300/30`}
